...
class RecurringInvoice(models.Model):
    ...
    def __str__(self):
        return f"Recurring - {self.customer.name}"
...
class Worker(models.Model):
    name = models.CharField(max_length=255)
    finger_no = models.CharField(max_length=50, unique=True)
    iqama_no = models.CharField(max_length=50, unique=True)
    ...
    def __str__(self):
        return f"{self.name} (#{self.finger_no})"
...
class SalarySheet(models.Model):
    ...
    def __str__(self):
        return f"{self.worker.name} - {self.month}"
...
class TransferSlip(models.Model):
    slip_no = models.CharField(max_length=100)
    date = models.DateField(default=timezone.now)
