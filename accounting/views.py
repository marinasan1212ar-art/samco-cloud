...
from .models import (
    ...
    UserProfile, BillOfMaterials, WorkOrder
)
...
def dashboard_view(request):
    ...
    invoices = Invoice.objects.all().order_by('-id')[:6]
    work_orders = WorkOrder.objects.all().order_by('-id')[:6]
...
# --- 🏭 MANUFACTURING DASHBOARD (BOM & WORK ORDERS) ---
def manufacturing_view(request):
    ...
    boms = BillOfMaterials.objects.all().order_by('bom_code')
    ...
    return render(request, 'accounting/manufacturing.html', {
        'company': company,
        'work_orders': work_orders,
        'boms': boms,
        'tot_produced': tot_produced,
        'tot_mfg_cost': tot_mfg_cost,
        'lang': lang,
        'is_ar': is_ar,
        'lang_dir': lang_dir
    })

# --- 📱 MOBILE LIVE IN-BROWSER CAMERA BARCODE SCANNER ---
def scanner_view(request):
    lang = request.session.get('lang', 'en')
    is_ar = (lang == 'ar')
    lang_dir = 'rtl' if is_ar else 'ltr'
    return render(request, 'accounting/scanner.html', {
        'lang': lang,
        'is_ar': is_ar,
        'lang_dir': lang_dir
    })
...
def custom_reports_view(request):
    ...
    if report_type == 'sales':
        ...
        for t in trans.order_by('-date'):
            ...
            data_rows.append({'col1': t.transfer_no, 'col2': f"{t.source_warehouse.name_en} ➔ {t.destination_warehouse.name_en}", 'col3': t.status, 'col4': str(t.date), 'amount': Decimal(item_count), 'vat': Decimal('0.00'), 'total': Decimal(item_count), 'link': f"/transfer/{t.id}/"})

    return render(request, 'accounting/custom_reports.html', {
        'company': company, 'report_type': report_type, 'start_date': start_date, 'end_date': end_date,
        'customers': customers, 'warehouses': warehouses, 'selected_customer': customer_id, 'selected_warehouse': warehouse_id,
        'data_rows': data_rows, 'tot_amount': tot_amount, 'tot_vat': tot_vat, 'tot_grand': tot_grand,
...
def reports_view(request):
    ...
    trial_balance = []
    tot_debit_all = Decimal('0.00'); tot_credit_all = Decimal('0.00')
...
def invoice_detail_view(request, pk):
    ...
    return render(request, 'accounting/invoice_detail.html', {'invoice': invoice, 'company': company, 'qr_image': qr_img})
...
def voucher_print_view(request, v_type, pk):
    ...
    return render(request, 'accounting/voucher_print.html', {'voucher': voucher, 'v_type': v_type, 'title_en': title_en, 'title_ar': title_ar, 'party_label': party_label, 'party_name': party_name, 'company': company})
...
def transfer_slip_print_view(request, pk):
    ...
    return render(request, 'accounting/transfer_print.html', {'transfer': transfer, 'company': company, 'qr_image': qr_img})
